...
class HouseParticipation < ApplicationRecord
  ...
  # @abstract removes points and then adds a different amount of points (when updating submission time)
  # @does not update if the submission is older than a month.
  def update_points(old_points, new_points, month)
    return unless month == Time.now.utc.month
